...
export const addCompetency = async (req, res, next) => {
	try {
		...
		if (admin.length === 0) {
			return res
				.status(404)
				.json({ message: "Data branch admin tidak ditemukan" });
		}

		// Validate and sanitize date_obtained
		const validDateObtained =
			date_obtained && date_obtained !== "" ? date_obtained : null;
...
// PUT /api/branch_admins/competencies/:id - Update competency
export const updateCompetency = async (req, res, next) => {
	try {
		const userId = req.user.id;
		// Support both REST (:id param) and legacy (body.id)
		const competencyId = req.params.id || req.body.id;
		const {
			competency_name,
			issuer,
			date_obtained,
...
// DELETE /api/branch_admins/competencies/:id - Delete competency
...
export const listMembersInBranch = async (req, res, next) => {
	try {
      ...
      SELECT 
        ...
        (SELECT payment_type FROM payment_proofs pp 
         WHERE pp.member_id = m.id AND pp.payment_month = ? AND pp.payment_year = ?) as current_month_payment_type
      FROM members m
      WHERE m.branch_id = ?
    `;
		const params = [
      ...
      ORDER BY m.full_name ASC`,
		...
		if (members.length === 0) {
			return res
				.status(404)
				.json({
					message: "Tidak ada member aktif untuk diekspor di cabang ini.",
				});
		}
