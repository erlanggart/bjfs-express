...
export const moveBranch = async (req, res, next) => {
  try {
    const { member_id, new_branch_id } = req.body;

    if (!member_id || !new_branch_id) {
      return res.status(400).json({ message: 'Member ID dan Branch ID diperlukan' });
    }

    await db.query(
      'UPDATE members SET branch_id = ? WHERE id = ?',
      [new_branch_id, member_id]
    );
...
export const resetPassword = async (req, res, next) => {
  try {
    ...
    const defaultPassword = 'bogorjunior123'; // Default password
...
export const getAttendanceHistory = async (req, res, next) => {
  try {
    ...
    const [attendance] = await db.query(`
      SELECT a.*, s.training_date, s.start_time, s.end_time, b.name as branch_name
      FROM attendance a
      LEFT JOIN schedules s ON a.schedule_id = s.id
      LEFT JOIN branches b ON s.branch_id = b.id
      WHERE a.member_id = ?
      ORDER BY s.training_date DESC, s.start_time DESC
      LIMIT ? OFFSET ?
...
export const createEvaluation = async (req, res, next) => {
  try {
    const { member_id, evaluation_date, report_type, scores, coach_notes } = req.body;
    ...
    const [result] = await db.query(
      ...
      [member_id, authorId, evaluation_date, report_type, JSON.stringify(scores), coach_notes]
    );
...
export const deleteEvaluation = async (req, res, next) => {
  try {
    ...
    await db.query('DELETE FROM member_evaluations WHERE id = ?', [evaluationId]);
...
export const addAchievement = async (req, res, next) => {
  try {
    ...
    if (!achievement_name || !event_date) {
      return res.status(400).json({ message: 'Nama prestasi dan tanggal diperlukan' });
    }
...
export const updateAchievement = async (req, res, next) => {
  try {
    ...
    if (achievements.length === 0 || achievements[0].member_id !== memberId) {
      return res.status(403).json({ message: 'Tidak memiliki akses untuk mengubah prestasi ini' });
    }
...
export const deleteAchievement = async (req, res, next) => {
  try {
    ...
    if (achievements.length === 0 || achievements[0].member_id !== memberId) {
      return res.status(403).json({ message: 'Tidak memiliki akses untuk menghapus prestasi ini' });
    }

    await db.query('DELETE FROM member_achievements WHERE id = ?', [achievementId]);
...
export const getMyPaymentHistory = async (req, res, next) => {
  try {
    ...
    const [payments] = await db.query(
      'SELECT * FROM payment_proofs WHERE member_id = ? ORDER BY payment_year DESC, payment_month DESC',
      [memberId]
    );
...
export const uploadPayment = async (req, res, next) => {
  try {
    ...
    const [members] = await db.query('SELECT id, full_name FROM members WHERE user_id = ?', [userId]);
    ...
    if (!paymentMonth || !paymentYear) {
      return res.status(400).json({ message: 'Periode pembayaran tidak valid.' });
    }
    ...
    const memberFolder = path.join(__dirname, '../../uploads/proofs', folderName);
    ...
    await db.query(
      `INSERT INTO payment_proofs (member_id, payment_month, payment_year, proof_url, status) 
       VALUES (?, ?, ?, ?, 'pending')
       ON DUPLICATE KEY UPDATE proof_url = VALUES(proof_url), status = 'pending'`,
      [memberId, paymentMonth, paymentYear, proofUrl]
    );
...
export const markAsPaid = async (req, res, next) => {
  try {
    ...
    await db.query(
      'UPDATE members SET last_payment_date = ? WHERE id = ?',
      [today, member_id]
    );
