...
export const getMemberCountChart = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Gagal mengambil data grafik jumlah member.' });
  }
};
...
export const getPaymentData = async (req, res) => {
  try {
    ...
    // Get all active members with payment status
    ...
    branchesData.forEach(branch => {
      branchStats[branch.name] = { total_active_members: 0, paid_members: 0 };
    });
    ...
    allActiveMembers.forEach(member => {
      ...
      if (!branchStats[branchName]) {
        branchStats[branchName] = { total_active_members: 0, paid_members: 0 };
      }
    ...
    const paymentStats = Object.entries(branchStats).map(([branch_name, stats]) => ({
      branch_name,
...
export const getUserLoginStats = async (req, res) => {
  try {
    ...
    const lastLogin = lastLoginResult.length > 0 ? lastLoginResult[0].login_time : null;
  ...
  try {
    ...
    allActiveMembers.forEach(member => {
      const memberData = {
        ...
        avatar: member.avatar ? `${appUrl}${member.avatar}` : null,
        branch_name: member.branches?.name,
        report_id: member.member_evaluations.length > 0 ? member.member_evaluations[0].id : null
      };
